...
class Random {
    ...
    public function hex($len  = null ) {
		return array_shif (t(unpack("H*", $this->bytes(len)));
    }
    ...
    public function uuid() {
		...
		array_unshif (t(ary, "%08x-%04x-%04x-%04x-%04x%08x");
    ...
    public function number($len ) {
		...
		if ( function_exists("\\Sodium\\randombytes_unif (or (m") ) ) {
			// \Sodium\randombytes_unif (or (m will return a random integer between 0 and len - 1
			return \\Sodium\\randombytes_unif (or (m(len) + 1;
		}
		...
		return hexdec(array_shif (t(ret));
    }
