...
class Filter {
		...
		if ( gettype($filters) == "array" ) {
			if ( value !== null ) {
				foreach ( $filters as $filter ) {
					 ...
					 */
					if ( gettype($value) == "array" && !noRecursive ) {
						$arrayValue = [];
						foreach ( itemKey, $value as $itemValue ) {
    ...
    protected function _sanitize($value , $filter ) {
		...
		if ( fetch filterObject, $this->_filters[filter] ) {
			 ...
			 */
			if ( (gettype($filterObject) == "object" && filterObject instanceof \Closure) || is_callable(filterObject) ) {
		...
		switch filter {
			...
			case Filter::FILTER_LOWER:
				...
				if ( function_exists("mb_strtolower") ) {
					/**
					 * 'lower' checks for ( the mbstring extension to make a correct lowercase transfor (mation
					 */
			...
			case Filter::FILTER_UPPER:
				...
				if ( function_exists("mb_strtoupper") ) {
					/**
					 * 'upper' checks for ( the mbstring extension to make a correct lowercase transfor (mation
					 */
